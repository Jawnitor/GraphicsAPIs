...
bool Shaderprogram::loadShaders(const char *vsFilename,
...
void Shaderprogram::use() {  // makes shader active
  if (mHandle > 0) glUseProgram(mHandle);
}

std::string Shaderprogram::fileToString(
    const std::string &filename) {  // reads the shader files
  std::stringstream ss;
  std::ifstream file;
